    ...
    for form in forms:
        ...
        for c in "\"'":
            ...
            if is_vulnerable(res):
                ...
                print("[+] Form:")
                pprint(form_details)
choice = input('InSp' + Fore.RED + '(option/SQLiScanner)' + Fore.WHITE + '>')
myTable = PrettyTable(["Command", "Info"])
myTable.add_row(["SUrl", "url (ex: www.site.com/php?id=1)"])
myTable.add_row(["run", " "])
if choice  == 'show options':
	...
	run = input('InSp' + Fore.RED + '(option/SQLiScanner/SUrl)' + Fore.WHITE + '>')
	if run == 'run':
